...
import type { CommandResult } from '@danielng23/dsh-mecha-host-system-metrics/types'
...
/**
 * The workspace inset below the center chrome: the center widget's 2px neon
...
export function EdexShell({
  ...
  useLayoutEffect(() => {
    ...
    // The empty top panel takes no space, so the center chrome (2px border +
    // 20px title strip) starts at the viewport top and the original UI mounts
    // below it: inset top by the strip, sides/bottom by the border width.
    frame.style.top = `calc(${TOP_BAR_HEIGHT} + ${CENTER_TITLE_INSET})`
    frame.style.left = `calc(${CENTER_LEFT} + ${CENTER_CHROME_INSET})`
    frame.style.right = `calc(${CENTER_RIGHT} + ${CENTER_CHROME_INSET})`
    frame.style.bottom = `calc(${CENTER_BOTTOM} + ${CENTER_CHROME_INSET})`
  ...
  return (
    ...
    >
      ...
      {/* The ORIGINAL UI wrapped in the standard widget chrome: a MECHA
          title strip (the same card header as the side widgets) above the
          reshaped layout frame, which mounts below it (CENTER_TITLE_INSET).
          The faint scanline texture overlays the region, click-through. */}
      <section className={css.centerWidget}>
        <div className={css.centerTitle} aria-hidden="true">
          MAGI SYSLINK — WORKSPACE
        </div>
