...
/** The default theme color — the MECHA klaxon red-orange. */
export const DEFAULT_THEME_COLOR = '#ff3300'
...
export const THEME_COLOR_PRESETS: readonly ThemeColorPreset[] = Object.freeze([
  { id: 'analyzed', labelKey: 'edex.preset.analyzed', color: '#ff3300' },
...
/** MECHA semantic accents measured from the reference (analysis.json theme.semanticColors). */
const MECHA_WARN = '#ffaa00'
const MECHA_ERROR = '#ff2533'
const MECHA_INFO = '#ff3300'
...
/** MECHA surfaces measured from the reference: pure-black canvas, near-black card body. */
const MECHA_PANEL_SURFACE = '#0a0b0e'
...
export function tokenOverridesFor(palette: EdexPalette): ThemeTokenOverrides {
  return {
    // The workspace canvas reads as the SAME surface as the shell panels: the
    // MECHA near-black card body (#0a0b0e) on every background token, never
    // pure black (the reference's cards sit on a black canvas as one surface).
    '--dsw-alias-bg-base': both(MECHA_PANEL_SURFACE),
    '--dsw-alias-bg-layer-1': both(MECHA_PANEL_SURFACE),
    '--dsw-alias-bg-layer-2': both(MECHA_PANEL_SURFACE),
    '--dsw-alias-bg-overlay': both(MECHA_PANEL_SURFACE),
    ...
    '--dsw-alias-state-success-primary': both(FIXED_ACCENTS.amber),
    ...
    '--dsw-specific-sidebar-fill': both(MECHA_PANEL_SURFACE),
    '--dsw-specific-input-major': both(MECHA_PANEL_SURFACE),
  }
}
